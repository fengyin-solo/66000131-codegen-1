        ...
        <div className="grid grid-cols-2 gap-2">
          {THEMES.map(t => (
            <button key={t.id} onClick={() => store.setTheme(t.id)}
              className={`flex items-center gap-1 px-2 py-1 rounded text-xs transition ${store.themeId===t.id?'bg-indigo-600 ring-2 ring-indigo-400':'bg-gray-700 hover:bg-gray-600'}`}>
